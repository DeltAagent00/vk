...
class NewsTableViewCell: UITableViewCell {
    ...
    @IBOutlet weak var repostView: UIButton?
    @IBOutlet weak var viewView: UIButton?
    @IBOutlet weak var newsImageViewHeightConstraint: NSLayoutConstraint?
    ...
    func fill(newsFeed: NewsFeed, owner: NewsFeedOwner, cacheImageService: CacheImageService?, indexPath: IndexPath) {
        ...
        if let photos = newsFeed.photos, let items = photos.items {
            ...
            newsImageViewHeightConstraint?.constant = 90
        } else {
        ...
        if let likes = newsFeed.likes {
            //likeView?.titleLabel?.text = "\(likes.count)"
            likeView?.setTitle("\(likes.count)", for: .normal)
        }
        if let comments = newsFeed.comments {
            commentView?.titleLabel?.text = "\(comments.count)"
        }
        if let reposts = newsFeed.reposts {
            repostView?.titleLabel?.text = "\(reposts.count)"
        }
        if let views = newsFeed.views {
            //viewView?.titleLabel?.text = "\(views.count)"
            viewView?.setTitle("\(views.count)", for: .normal)
        }
